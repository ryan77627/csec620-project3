<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!-- original declaration <xsl:template match="disk[target/@bus='ide']">
    <xsl:copy>
      <xsl:apply-templates select="@*"/>
      <xsl:variable name="targetIndex" select="position()"/>
  ...
  <!-- Adjusted declaration to support multiple IDE disks -->
  <xsl:template match="disk[target/@bus='ide']">
    <xsl:copy>
      <xsl:apply-templates select="@*"/>
      <xsl:variable name="idePos" select="count(preceding-sibling::disk[target/@bus='ide']) + 1"/>
      <xsl:variable name="devLetter" select="substring('cdefghijklmnop', $idePos, 1)"/>
      <target dev="sd{$devLetter}" bus="sata"/>
      <xsl:apply-templates select="node()[not(self::target)]"/>
    </xsl:copy>
  </xsl:template>
</xsl:stylesheet>
